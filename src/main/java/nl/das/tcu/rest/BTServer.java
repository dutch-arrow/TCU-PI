	...
	public static void handleCommand (String command, OutputStream out) throws IOException {
		...
		try {
			...
			case "setDeviceOn": {
				...
				Terrarium.getInstance().setDeviceOn(prm, DeviceState.ENDTIME_INDEFINITE, DeviceState.NOT_CONTROLLED);
				res.setResponse(JsonObject.EMPTY_JSON_OBJECT);
				break;
			}
			case "setDeviceOff": {
				...
				Terrarium.getInstance().setDeviceOff(prm, DeviceState.NOT_CONTROLLED);
				res.setResponse(JsonObject.EMPTY_JSON_OBJECT);
				break;
			}
			case "setDeviceOnFor": {
				...
				Terrarium.getInstance().setDeviceOn(prm, Util.now(LocalDateTime.now()) + per, 4);
				res.setResponse(JsonObject.EMPTY_JSON_OBJECT);
				break;
			}
...
//				Terrarium.getInstance().setNow(LocalDateTime.now());
			...
			case "getTemperatureRule": {
				int prm = cmd.getData().getInt("rulenr", 0);
				if( prm == 0) {
					throw new CommandException("Integer parameter 'rulenr' not found.");
				}
				JsonReader jsonReader = Json.createReader(new StringReader(jsonb.toJson(Terrarium.getInstance().getRule(prm))));
				JsonObject object = jsonReader.readObject();
				res.setResponse(object);
				break;
			}
			case "saveTemperatureRule": {
				JsonObject obj = cmd.getData().get("rule").asJsonObject();
				if (obj == null) {
					throw new CommandException("JsonObject parameter 'rule' not found.");
				}
				int prm = cmd.getData().getInt("rulenr", 0);
				if( prm == 0) {
					throw new CommandException("Integer parameter 'rulenr' not found.");
				...
				try {
					rule = jsonb.fromJson(obj.toString(), TemperatureRule.class);
					Terrarium.getInstance().replaceRule(prm, rule);
					Terrarium.getInstance().saveSettings();
					res.setResponse(JsonObject.EMPTY_JSON_OBJECT);
				} catch (JsonbException e) {
					throw new CommandException("JsonObject parameter 'rule' does not contain a TemperatureRule json object.");
				}
				break;
			}
			case "getSprayerRule": {
